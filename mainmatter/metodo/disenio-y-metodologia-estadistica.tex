...
\subsection{Asignaci\'on de valores de etiquetas y rangos de identificaci\'on}
...
\end{formula} 
\end{itemize}
\subsection{C\'alculo indirecto de la altura del usuario}
...
\item Potencia
\end{itemize}
\subsection{Captura de datos durante una rutina}
...
\end{figure}
\subsection{Validaci\'on del modelo de reconocimiento de movimiento}
...
\begin{table}[H]
\begin{center}
\caption{Validaci\'on cruzada, K-fold,  Tenis de mesa}
...
\begin{table}[H]
\begin{center}
\caption{Validaci\'on cruzada, K-fold,  Animaci\'on}
...
\begin{table}[H]
\begin{center}
\caption{Validaci\'on cruzada, K-fold,  Taekwondo}
...
\begin{itemize}
\item \textbf{Error Medio Pron\'osticado (EMP):} Promedio de diferencia entre el valor real y pron\'osticado, la cual puede tener tres interpretaciones distintas:
	\begin{itemize}
...
\end{formula}  
\item \textbf{Desviaci\'on Media absoluta (MAD):} Promedio de diferencia absoluta entre el valor real y pron\'osticado, que muestra la dispersi\'on con respecto al valor real.
 \begin{formula}[H]
	\centering
	\caption{C\'alculo de la desviaci\'on media absoluta}
	\label{frm:MADMath}
	\begin{equation}
MAD=\frac{\sum_{0}^{n}( \left |  Real_{x}-Pronostic_{x}\cdot10^{-6} \right |)}{n}
	\end{equation}
	\textbf{Fuente:} Formula adaptada al proyecto, a partir de la f\'ormula MAD \cite{videoErrores}
\end{formula}  
...
\end{formula}  
\end{itemize}
Luego de encontrar los errores por cada modelo, se debe seleccionar el modelo que tenga el menor error \acrshort{RECM}, y posteriormente encontrar los errores de la muestra total:
 \begin{formula}[H]
	\centering
...
\begin{figure}[H]
	...
	\textbf{Fuente:} Elaborado por el autor de tesis
\end{figure}
Finalmente, cada modelos aprobado se debe encontrar el factor de reconocimiento y sus respectivo rango de confiabilidad:
\begin{formula}[H]
	\centering
	\caption{Rango de confiabilidad}
	\label{frm:RecmAll}
	\begin{equation}
\begin{matrix}
